...
from .constants import (
    ...
    OBSTACLE_CONFIRMATION_FRAMES,
    OBSTACLE_CONFIRMATION_HITS,
    TOF_CHANNELS_HYPERION_DEFAULT,
    TOF_CHANNELS_MAX,
    TOF_DEFAULT_PORT,
    TOF_THRESHOLD_MM,
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            f"Installed ToF channels. Default: {TOF_CHANNELS_DEFAULT}. "
            "The host switches ACT 2/ACT 4 at runtime."
        ),
    )
    parser.add_argument(
        "--hyperion-mode",
        action="store_true",
        help="Compatibility flag; 4-sensor Hyperion mode is now the default when ToF is enabled.",
    )
    ...
    parser.add_argument("--detect-threshold-mm", type=float, default=TOF_THRESHOLD_MM,
                        help=f"CH0/CH1 detection threshold in mm (default: {TOF_THRESHOLD_MM:.0f})")
    parser.add_argument("--confirm-frames", type=int, default=OBSTACLE_CONFIRMATION_FRAMES,
                        help=f"Confirmation sample window (default: {OBSTACLE_CONFIRMATION_FRAMES})")
    parser.add_argument("--confirm-hits", type=int, default=OBSTACLE_CONFIRMATION_HITS,
                        help=f"Hits required within confirmation window (default: {OBSTACLE_CONFIRMATION_HITS})")
    ...
    default_channels = TOF_CHANNELS_HYPERION_DEFAULT
    channel_cap = TOF_CHANNELS_MAX
    requested_channels = default_channels if args.tof_channels is None else int(args.tof_channels)
    tof_channels = max(1, min(requested_channels, channel_cap))
    hyperion_mode = bool((not args.no_tof) and tof_channels >= 4)
    ...
    if teensy_port:
        ...
        print(f"Sensor layout mode: {'HYPERION-4' if hyperion_mode else 'DUAL-2'}")
        print(f"Installed ToF channels: {tof_channels} (runtime switches ACT 2/ACT 4)")
        print(f"Detect threshold: {float(args.detect_threshold_mm):.0f} mm")
        print("Planner model: vertical_probe_sector_waypoints_v2")
    else:
    ...
    ctrl = BraccioController(
        ...
        obstacle_class="SECTOR",
        hyperion_mode=hyperion_mode,
        detect_threshold_mm=float(args.detect_threshold_mm),
    ...
    print("Press ESC to quit. R=side profile, B=record, C=IMU calibrate.\n")
