...
# Wrist-vertical override for the simple-replan sweep. When not None,
# replaces the IK-auto-leveled wrist_vert (= shoulder + elbow - 180) on
# every sweep advance / detour waypoint. Set to 0 to pin the gripper
# horizontal in the sweep test; bump up/down 5° at a time to tune. Leave
# ``None`` to fall back to the IK solver's default level angle.
SWEEP_WRIST_V_DEG = 0  # int or None; None → auto-level via IK
...
# Referenced by rl_recorder.py, rl_reward.py, rl_env.py, calibrate_noise.py.
# These are kept separate from the safety-stack constants above so the RL
# pipeline can be imported without pulling in behaviour-tree dependencies.

# Sweep tick rate used by the RL recorder (matches BT_TICK_HZ in practice
# but decoupled so the recorder can be adjusted independently).
SWEEP_TICK_HZ = BT_TICK_HZ   # 20 Hz default

# Discrete Z levels tried during RL Z-level reachability mask encoding.
# Aligned with SWEEP_Z_LADDER_MM so the policy mask reflects real choices.
SWEEP_Z_CANDIDATES = SWEEP_Z_LADDER_MM   # [35, 60, 90, 10, -20] mm
...
OBS_MAP_GRID_SIZE = 8    # VL53L5CX 8×8 sensing grid
...
# Persistent obstacle memory voxel grid parameters.
OBS_MEM_CELL_MM            = 40.0
OBS_MEM_MAX_CELLS          = 2000
OBS_MEM_INC                = 0.30
OBS_MEM_DECAY_PER_SEC      = 0.15
OBS_MEM_KEEP_THRESHOLD     = 0.05
OBS_MEM_OCCUPIED_THRESHOLD = 0.20

# IR emergency stop — hardware is 4 sensors wired as 2 NOR-gate pairs →
# binary output (0 or 3).  IR_MONITOR_HZ runs a dedicated daemon faster
# than the main BT tick so the arm halts within ~20 ms, not one tick.
IR_MIN         = 1    # ir_bits >= IR_MIN → emergency stop
IR_MONITOR_HZ  = 50   # Hz for the dedicated IR-monitor thread

# RL human feedback key bindings (injected into KEY_BINDINGS below at runtime
# if the controller detects an active RLRecorder).
# R → reward positive, Y → reward negative (T/t already taken by strict_toggle).
RL_KEY_FEEDBACK_POS = ord('r')   # R key → add_human_feedback(+0.5)
RL_KEY_FEEDBACK_NEG = ord('y')   # Y key → add_human_feedback(-0.5)
