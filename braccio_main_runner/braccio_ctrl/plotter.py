...
class ArmPlotter:
    ...
    def __init__(
        ...
        self._log_writer: Any = None  # csv.writer
        self._log_lock = threading.Lock()

        # ── Lifecycle ─────────────────────────────────────────────────────
        self._stop_event = threading.Event()
        self._plt: ModuleType | None = None  # pyplot module, set after GUI init
        self._ui_ready = False
        self._ui_thread: threading.Thread | None = None
        self._ui_thread_id: int | None = None
        self._ui_cmd_q: queue.SimpleQueue = queue.SimpleQueue()

        # ── Main combined figure (6 subplots, 2×3 grid) ───────────────────
        self._fig: Figure | None = None  # Figure — also used by save_screenshot()
        self._axes: list[Axes] = []  # filled in _build_main_figure()
        self._lines: list[Line2D] = []  # filled in _build_main_figure()
        self._anim = None  # FuncAnimation instance
        self._status_txt: Text | None = None  # logging indicator text artist

        # ── Per-joint pop-out figures ─────────────────────────────────────
        self._ind_figs: list[Figure | None] = [
            None for _ in range(6)
        ]  # Figure per joint, or None if closed
        self._ind_axes: list[Axes | None] = [
            None for _ in range(6)
        ]  # list[Axes], length 6, in joint order
        self._ind_lines: list[Line2D | None] = [
            None for _ in range(6)
        ]  # list[Line2D], one per joint
    ...
    def toggle_logging(self) -> bool:
        ...
        with self._log_lock:
            if self._logging:
                ...
                self._log_writer.writerow(["time_s"] + [n.strip() for n in JOINT_NAMES])
                self._logging = True
    ...
    def _ui_loop(self) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            try:
                ...
                for fig in self._ind_figs:
                    if fig is not None and self._plt.fignum_exists(
                        figure_number(fig)
                    ):
    ...
    def _drain_ui_commands(self) -> None:
        ...
        while True:
            try:
                ...
                return
            if not self._ui_ready and cmd in (
                "toggle_main",
                "toggle_joint",
                "screenshot",
            ):
    ...
    def _build_ind_figure(self, i: int):
        ...
        plt = self._plt
        if plt is None:
            raise RuntimeError("plotter: pyplot not initialised")
        lo, hi = JOINT_LIMITS[i]
    ...
    def _update(self, _frame) -> list:
        ...
        for i in range(6):
            ...
            if plt.fignum_exists(figure_number(fig)):
                ind_line = self._ind_lines[i]
                ind_ax = self._ind_axes[i]
                if ind_line is None or ind_ax is None:
                    continue
                ind_line.set_data(t_data, a_data[i])
                ind_ax.set_xlim(x_min, x_max)
                fig.canvas.draw_idle()
            else:
    ...
    def _toggle_main_window(self) -> None:
        ...
        if self._fig is None:
            return
        try:
            win: Any = getattr(self._fig.canvas.manager, "window", None)
            if win is None:
                return
            # Tk backend
            try:
