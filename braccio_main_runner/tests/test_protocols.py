...
def test_parse_tf_hyperion_4x4():
    d = ",".join(["150"] * 16)
    v = ",".join(["1"] * 16)
    line = f"TF,7,2222,S2,2,wrist_pitch,0,4,4,{d},{v}"
    fr = parse_tof_line(line)
    assert fr is not None
    assert fr.mux_channel == 2
    assert fr.rows == 4
    assert fr.cols == 4
    assert len(fr.distances_mm) == 16
    assert len(fr.validity) == 16
